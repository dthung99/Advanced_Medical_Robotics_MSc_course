...
class PIDAngleControl_2(Node):
    def __init__(self):
        super().__init__('pid_angle_control_2')  # Initialize the node with the name 'simple_publisher'
        # Declare parameters
        ...
        self.pid_send_current = self.create_publisher(Float32MultiArray,'/joint_cur_2',1)
    ...
    def control_function(self, msg):
        self.current_velocity = np.array(msg.data)[4]
...
def main(args=None):
    ...
    pid_angle_control = PIDAngleControl_2()  # Create an instance of the SimplePublisher
    print("Node started")
    try:
