...
class PIDAngleControl(Node):
    def __init__(self):
        ...
        # Declare parameters
        self.friction_coef_1 = np.array([1.0, 5.2785, 1.0031])
        self.friction_coef_2 = np.array([5.9386, 25.9792, 4.5751])
        self.kp = np.array([1.5, 1.5, 1.5])
        self.ki = np.array([10, 30, 10])
        self.kd = np.array([0, 0, 0])
        self.current_limit = np.array([500, 500, 500])
