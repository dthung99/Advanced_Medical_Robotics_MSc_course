...
# NEWWWWWWWWWWWWWWW 
    def get_Weighted_Velocity_needed_for_Target_From_Task_Space_Difference(self,
                                                                           difference_in_task_space: np.ndarray,
                                                                           d_error_clamping: float = np.finfo(float).max,
                                                                           weight_vector: np.ndarray = np.array([1, 1, 1]),
                                                                           k_damped_factor: float = 0,
                                                                           ) -> np.ndarray:
        ...
        # Calculate the pseudo inverse: J^T*inv(J*JT)
        pseudo_inv = np.matmul(matrix_a, matrix_b)
        task_space_vel = self.error_clamping(difference_in_task_space, d_error_clamping)
        task_space_vel = task_space_vel[0:2]
        # Calculate the target velocity
        joint_space_vel = np.matmul(pseudo_inv,task_space_vel.reshape(-1,1)).flatten()
        return joint_space_vel
...
class Solver:
    ...
    # # NEW
    # def Equality_for_Forward_Kinematics_x(self, x):
    #     l = self.arm_length
    #     return np.cos(x[0])*l[0] + np.cos(x[0]+x[1])*l[1] + np.cos(x[0]+x[1]+x[2])*l[2] - self.end_effector_target[0]
    # def Equality_for_Forward_Kinematics_y(self, x):
    #     l = self.arm_length
    #     return np.sin(x[0])*l[0] + np.sin(x[0]+x[1])*l[1] + np.sin(x[0]+x[1]+x[2])*l[2] - self.end_effector_target[1]
    # def Total_Square_Distance_to_Center(self,
    #                                     x: np.ndarray) -> np.float64:
    #     middle_point = (self.joint_upper_limit + self.joint_lower_limit)/2
    #     x = (x - middle_point)/(self.joint_upper_limit - self.joint_lower_limit)
    #     x = (x+180)%360-180
    #     x = np.sum(x**2)
    #     return x
    # def Set_Variable_for_Maximizing_Distance_to_Joint_Limit_Solver(self,
    #                                                                arm_length: np.ndarray = np.array([100, 100, 100]),
    #                                                                joint_lower_limit: np.ndarray = np.deg2rad([-100,-100,-100]),
    #                                                                joint_upper_limit: np.ndarray = np.deg2rad([100,100,100])):
    #     self.arm_length = arm_length
    #     self.joint_lower_limit = joint_lower_limit
    #     self.joint_upper_limit = joint_upper_limit
    #     return
    # def Solving_Inverse_Kinematics_while_Maximizing_Distance_to_Joint_limit(self,
    #                                                                         end_effector_target: np.ndarray,
    #                                                                         initial_joint_angle: np.ndarray = np.array([0,0,0])) -> np.ndarray:
    #     self.end_effector_target = end_effector_target
    #     constraints = [{'type': 'eq', 'fun': self.Equality_for_Forward_Kinematics_x},
    #                    {'type': 'eq', 'fun': self.Equality_for_Forward_Kinematics_y}]
    #     result = minimize(fun=self.Total_Square_Distance_to_Center,
    #                       x0=initial_joint_angle,
    #                       constraints=constraints,
    #                       method='SLSQP',
    #                       tol = 0.01)
    #     result = result.x
    #     result = result - ((result+np.pi)//(2*np.pi)).astype(int)*np.pi*2
    #     return result 
