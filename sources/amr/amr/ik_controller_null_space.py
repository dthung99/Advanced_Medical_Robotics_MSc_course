...
class IkController_null_space(Node):

    def __init__(self):
        super().__init__('ik_controller_null_space')  # Initialize the node with the name 'simple_subscriber'
        ...
        self.q_0 = np.array([20, 20, 20])
        self.target = np.array([299, 0, 10])
    ...
    def listener_callback(self, msg):
        ...
        Pseudo_Inverse_Jacobian = np.matmul(J.T,np.linalg.inv(np.matmul(J,J.T)))

        target_velocity = np.matmul(np.eye(3) - np.matmul(Pseudo_Inverse_Jacobian, J), self.q_0.reshape(-1,1)).reshape(-1,)
        target_velocity = -target_velocity*180/np.pi
        Position_msg = Float32MultiArray()
        Position_msg.data = target_velocity.tolist()
...
def main(args=None):
    ...
    simple_subscriber = IkController_null_space()  # Create an instance of the SimpleSubscriber
